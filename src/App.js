...
// --- SENARAI STAF ---
const senaraiStaf = [
  "Abdul Hamid bin Sakmud @ Abdullah", "Adiniah Binti Muhamad Radzai", "Ahmad Fadhlullah bin Rusaidi",
  "Anamary binti Madyusah", "Andrew Bin Arih", "Anzari bin Mohd Daud", "Aslinah binti Aldan",
  "Asriyani binti Seraila", "Azryzan bin Besri", "Azwie bin Jafri", "Billy Anak Rejap",
  "Darman bin Daming", "Faten Farhana binti Wong", "Hafizulhaq bin Hatib", "Haslinda binti Bohari",
  "Hazrudy bin Ahmad Nasaruddin", "Ibrahim bin Lamusa", "Isabella Francis Xavier", "Ismail Bin Muin",
  "Jaikol bin Udar", "Jamludin bin Assat", "Japri Bin Patomdang", "Juraini binti Sahid",
  "Lynn Noell Ending", "Mohamad Sali bin Saleh", "Mohammad Nasir bin Awang", "Mohd Hafizul bin Ibrahim Apani",
  "Mohd Hairi bin Mohd Shah", "Mohd Hakimin Mohd Hussin", "Mohd Nur Fitri bin Jamil", "Mohd Shamin bin Ahmad",
  "Muhaidi bin Mohamad", "Muhammad Alinafiah bin Sabril", "Muhalis bin Nonchi", "Nadzihah binti Ahmad",
  "Nasri bin Kipple", "Nazriati binti Nasib", "Nazry bin Yusof", "Noraini binti Sukri",
  "Norashikin Binti Ariffin", "Norashsikin binti Mohd Arsad", "Norhadzla binti Abd Halim", "Nur Syafiqah binti Arman",
  "Nurulizaty binti Ibrahim", "Omrei bin Okong", "Richard Joanes", "Roha binti Awang Latif",
  "Rohana binti Ahmad", "Roshayati binti Mohammad", "Rusyieni @ Wendy Binti Payah", "Sakinah binti Pitungut",
  "Satria binti Murtala", "Shaharul bin Abu Talib", "Tc. Johannes Belili", "Tc. Mohd Radznan bin Malek",
  "Tc. Mohd Sabri bin Mohd Sarif", "Tc. Ng Vui Chien", "Tc. Silvester bin Lawai", "Ts. Joey Eriksen Teo",
  "Ts. Muhammad Haziq bin Hamzah", "Ts. Muhammad Hifzan bin Salimun", "Ts. Nurzharfan bin Rafei Bui",
  "Ts. Suhaidi bin Mustar", "Ts. Syed Mohd Yusri bin Syed Yusoff", "Zuliza binti Roslan"
];
...
const LiveClock = React.memo(() => {
  ...
  return (
    <div className="flex flex-col items-center justify-center my-3">
      <div className="flex items-baseline gap-1.5 font-mono font-black tracking-tighter text-3xl md:text-5xl text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">
        {timeNums}
        <span className="text-base md:text-xl font-extrabold text-blue-300 tracking-widest drop-shadow-none">
          {amPm}
        </span>
      </div>
      <div className="flex items-center gap-1.5 mt-2 px-3 py-1 bg-white/10 rounded-full border border-white/20 backdrop-blur-md">
...
export default function App() {
  ...
  // --- STATE BAHARU: KAWALAN ACCORDION BIRO ---
  ...
  const navigateTo = useCallback((view) => {
    ...
    setCurrentView(view);
    setOpenBiroIndex(null); // Reset/Tutup semua biro apabila tukar tab menu
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg text-xs font-extrabold text-white transition-all backdrop-blur-md shadow-sm border border-white/20 focus:ring-2 focus:ring-white/50 outline-none"
        >
  ...
  // --- KOMPONEN ACCORDION TERKINI (Terima prop dari luar) ---
  ...
  const SkeletonLoader = () => (
    <div className="p-4 max-w-5xl mx-auto pb-32 w-full space-y-6 mt-4">
      <div className="h-64 bg-slate-200 dark:bg-slate-800 rounded-[3rem] w-full animate-pulse transition-all duration-300"></div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {[1,2,3].map(i => <div key={i} className="h-48 bg-slate-200 dark:bg-slate-800 rounded-[2.5rem] w-full animate-pulse transition-all duration-300 delay-75"></div>)}
      </div>
    </div>
  );
          ...
          <div className={`max-w-6xl mx-auto px-4 lg:px-8 flex items-center justify-between transition-all duration-300 ease-out ${isScrolled ? 'h-16 md:h-18' : 'h-20 md:h-24'}`}>
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                <img src="https://i.ibb.co/6P0q5sP/Logo-ADTEC-JTM-2025-Kampus-Sandakan.png" alt="Logo" className={`w-auto object-contain bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 transition-all duration-300 ${isScrolled ? 'h-10 md:h-12 p-1.5' : 'h-12 md:h-14 p-2'}`} />
                <div>
              ...
              {currentView === 'home' && (
                <div className="px-4 lg:px-8 max-w-7xl mx-auto pb-32 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[
                      { id: 'memo', icon: FileText, title: 'Memo Lantikan', desc: 'Rujukan surat lantikan rasmi', color: 'blue' },
                      { id: 'ajk', icon: Users, title: 'Senarai AJK', desc: 'Jawatankuasa induk & biro', color: 'emerald' },
                      { id: 'jadual', icon: Calendar, title: 'Jadual MSR', desc: 'Tentatif terperinci program', color: 'orange' },
                      { id: 'penutup', icon: Award, title: 'Majlis Penutup', desc: 'Atur cara majlis perasmian', color: 'rose' },
                      { id: 'layout', icon: Map, title: 'Pelan Daftar', desc: 'Susun atur dewan pendaftaran', color: 'purple' },
                      { id: 'lagu', icon: Music, title: 'Lagu Korporat', desc: 'Lirik rasmi nyanyian JTM', color: 'pink' }
                    ].map((item) => (
                      <button 
                        key={item.id} 
                        onClick={() => navigateTo(item.id)} 
                        className="relative group overflow-hidden bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 text-left flex flex-col justify-between min-h-[240px] focus:outline-none focus:ring-4 focus:ring-blue-500/20"
                      >
                        <div className={`absolute -right-12 -top-12 w-40 h-40 ${cardStyles[item.color].blob} rounded-full blur-3xl group-hover:scale-150 group-hover:opacity-70 transition-all duration-700 ease-out`}></div>
                        <div className={`relative z-10 ${cardStyles[item.color].bg} p-4 rounded-2xl w-fit ${cardStyles[item.color].text} group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500 border ${cardStyles[item.color].border} shadow-sm`}>
                          <item.icon size={32} strokeWidth={2} />
                        </div>
                        <div className="relative z-10 mt-8">
                          <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-2 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">{item.title}</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium flex items-center">
                            {item.desc}
                            <ChevronRight size={18} className="opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-1 transition-all duration-300 text-blue-500 dark:text-blue-400" />
                          </p>
                        </div>
                      </button>
                    ))}
                    ...
                    <div className="space-y-4">
                      {biroList.map((biro, idx) => (
                        isAdmin ? (
                          ...
                          // PENTING: Penggunaan Komponen Accordion yang baharu
                          <AccordionBiro 
                            key={idx} 
                            biro={biro} 
