...
// --- LIVE CLOCK COMPONENT (MODERN & CENTERED) ---
const LiveClock = () => {
  ...
  // Asingkan masa (nombor) dan format (AM/PM)
...
export default function App() {
  ...
  // Data States
  const [announcement, setAnnouncement] = useState('🌟 Hari Pendaftaran Pelajar Baharu bermula 6 Julai 2026 jam 9:00 Pagi. Sila lapor diri di Dewan Besar ADTEC Sandakan.');
  ...
  const [ajkInduk, setAjkInduk] = useState(initialAjkInduk);
  const [biroList, setBiroList] = useState(initialBiroList);
  const [jadualData, setJadualData] = useState(initialJadual);
  const [activeJadualTab, setActiveJadualTab] = useState(initialJadual[0]?.id);
  const [penutupData, setPenutupData] = useState(initialPenutup);
  ...
  useEffect(() => {
    ...
    // Scroll listener for dynamic floating header
    const handleScroll = () => {
  ...
  // --- NAVIGATION WRAPPER (With Skeleton) ---
  ...
  const handleDocumentUpload = (e) => {
    ...
    if (file) {
      if (file.type === 'application/pdf') {
        setMemoFileUrl(URL.createObjectURL(file));
        setMemoFileType('pdf');
      } else if (file.type.startsWith('image/')) {
        const reader = new FileReader();
        reader.onload = (ev) => { setMemoFileUrl(ev.target.result); setMemoFileType('image'); };
  ...
  const handleLayoutUpload = (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
      const reader = new FileReader();
      reader.onload = (ev) => setLayoutImage(ev.target.result);
      reader.readAsDataURL(file);
    }
  };
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''}`}>
      <div className="min-h-screen bg-slate-100 dark:bg-slate-900 font-sans text-slate-900 dark:text-slate-100 flex flex-col relative transition-colors duration-500 selection:bg-blue-200 dark:selection:bg-blue-900">
        ...
        {/* --- TOP HEADER (Dynamic Glassmorphism & Hover Effect) --- */}
        <header className={`sticky top-0 z-40 backdrop-blur-xl transition-all duration-300 ${
          ...
          <div className={`max-w-6xl mx-auto px-4 flex items-center justify-between transition-all duration-300 ${
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <button 
                onClick={toggleTheme} 
                className="p-2 rounded-full bg-slate-200/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all active:scale-90"
              >
        ...
        {/* --- LOGIN MODAL (Glassmorphism + Dark Mode) --- */}
        {showLogin && (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
              ...
              {currentView === 'home' && (
                <div className="p-4 max-w-5xl mx-auto pb-32 space-y-5">
                  ...
                  <div className="bg-gradient-to-br from-blue-700 via-indigo-800 to-slate-900 dark:from-slate-800 dark:via-blue-950 dark:to-black rounded-[2rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden group">
                    {/* The new network canvas animation */}
                    <NetworkAnimation />
                    
                    {/* Abstract Floating Shapes for "Gempak" effect */}
                          ...
                          {isAdmin ? (
                            <div className="flex items-center gap-2">
                              ...
                              <ModernDropdown 
                                ...
                                onChange={(val) => setSesiKemasukan({ ...sesiKemasukan, sesi: val })}
                              />
                              ...
                              <ModernDropdown 
                                ...
                                onChange={(val) => setSesiKemasukan({ ...sesiKemasukan, tahun: val })}
                              />
                            </div>
                          ) : (
              ...
              {currentView === 'memo' && (
                <div className="p-4 max-w-4xl mx-auto pb-32">
                  <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors">
                    <div className="bg-slate-50 dark:bg-slate-800/80 p-5 md:p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                      <div>
                        <h3 className="font-extrabold text-slate-800 dark:text-white text-xl flex items-center gap-3">
                          <div className="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg text-blue-600 dark:text-blue-400"><FileText size={20}/></div>
                          Dokumen Memo
                        </h3>
                      </div>
                      {isAdmin && <span className="bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 px-3 py-1.5 rounded-lg text-xs font-bold animate-pulse">Mod Edit</span>}
                    </div>
                    
                    <div className="p-6 md:p-8">
                      {isAdmin && (
                        <div className="mb-8 p-6 bg-slate-50 dark:bg-slate-900/50 border-2 border-dashed border-blue-300 dark:border-blue-700 rounded-3xl relative group text-center hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                          <label className="flex flex-col items-center justify-center cursor-pointer">
                            <div className="bg-blue-100 dark:bg-blue-900/50 p-4 rounded-full mb-3 group-hover:scale-110 transition-transform">
                              <UploadCloud size={32} className="text-blue-600 dark:text-blue-400"/>
                            </div>
                            <span className="text-base font-extrabold text-slate-800 dark:text-slate-200 mb-1">Muat Naik Fail Memo / Surat</span>
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Sokongan: PDF / JPG / PNG</span>
                            <input type="file" accept="application/pdf, image/png, image/jpeg" className="hidden" onChange={handleDocumentUpload} />
                          </label>
                        </div>
                      )}

                      {memoFileUrl ? (
                        <div className="mb-6 relative rounded-3xl overflow-hidden shadow-md border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900">
                          {memoFileType === 'pdf' ? (
                          ...
                          {isAdmin && (
                            <button onClick={() => { setMemoFileUrl(null); setMemoFileType(null); }} className="absolute top-4 right-4 bg-red-500 text-white p-3 rounded-2xl shadow-lg hover:bg-red-600 active:scale-90 transition-all">
                              <Trash2 size={20}/>
                            </button>
                          )}
                        </div>
                      ) : (
                        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 md:p-10 min-h-[500px]">
                          {isAdmin ? (
                            <div className="h-full flex flex-col">
                              <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2"><Edit3 size={14}/> Sunting Teks Memo Manual</label>
                              <textarea 
                                value={memoText} 
                                onChange={(e) => setMemoText(e.target.value)} 
                                className="w-full flex-grow p-6 border-2 border-slate-200 dark:border-slate-600 rounded-2xl min-h-[400px] text-sm md:text-base font-medium focus:ring-0 focus:border-blue-500 outline-none leading-relaxed text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800 transition-colors custom-scrollbar" 
                              />
                            </div>
                          ) : (
                            <div className="prose prose-slate dark:prose-invert max-w-none text-sm md:text-base font-medium leading-loose whitespace-pre-wrap text-slate-800 dark:text-slate-300">
                              {memoText}
                            </div>
                          )}
                        </div>
              ...
              {currentView === 'ajk' && (
                <div className="p-4 max-w-4xl mx-auto pb-32">
                  ...
                  <div className="flex justify-between items-end mb-5 px-2">
                    ...
                    {isAdmin && (
                      <button onClick={() => setAjkInduk([...ajkInduk, { peranan: "Peranan Baru", nama: "Nama Baru" }])} className="text-sm bg-blue-600 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 font-bold hover:bg-blue-700 shadow-md transition-all active:scale-95">
                    ...
                    <div className="space-y-4">
                      {ajkInduk.map((ajk, idx) => (
                        <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-700 pb-4 last:border-0 last:pb-0">
                          {isAdmin ? (
                            <>
                              <input value={ajk.peranan} onChange={e => { const newAjk = [...ajkInduk]; newAjk[idx].peranan = e.target.value; setAjkInduk(newAjk); }} className="px-4 py-2.5 border-2 border-slate-200 dark:border-slate-600 rounded-xl text-sm w-full sm:w-1/3 font-bold uppercase bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-blue-500" />
                              <input value={ajk.nama} onChange={e => { const newAjk = [...ajkInduk]; newAjk[idx].nama = e.target.value; setAjkInduk(newAjk); }} className="px-4 py-2.5 border-2 border-slate-200 dark:border-slate-600 rounded-xl text-sm w-full font-bold bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-blue-500" />
                              <button onClick={() => setAjkInduk(ajkInduk.filter((_, i) => i !== idx))} className="text-red-500 p-3 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl transition-colors"><Trash2 size={20}/></button>
                            </>
                          ) : (
                            <>
                  ...
                  <div className="flex justify-between items-end mb-5 px-2">
                    ...
                    {isAdmin && (
                      <button onClick={() => setBiroList([...biroList, { nama: "Biro Baru", ketua: "Ketua", ahli: [] }])} className="text-sm bg-emerald-600 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 font-bold hover:bg-emerald-700 shadow-md transition-all active:scale-95">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      {biroList.map((biro, idx) => (
                        <div key={idx} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-[2rem] overflow-hidden shadow-sm p-6 relative transition-colors">
                          <button onClick={() => setBiroList(biroList.filter((_, i) => i !== idx))} className="absolute top-5 right-5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 p-2 rounded-xl transition-colors"><Trash2 size={20}/></button>
                          ...
                          <input value={biro.nama} onChange={e => { const newBiro = [...biroList]; newBiro[idx].nama = e.target.value; setBiroList(newBiro); }} className="w-[85%] px-4 py-2.5 mb-4 border-2 border-slate-100 dark:border-slate-700 rounded-xl font-extrabold text-sm bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-blue-500" />
                          ...
                          <input value={biro.ketua} onChange={e => { const newBiro = [...biroList]; newBiro[idx].ketua = e.target.value; setBiroList(newBiro); }} className="w-full px-4 py-2.5 mb-5 border-2 border-slate-100 dark:border-slate-700 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-blue-500" />
                          ...
                          <div className="bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700">
                            <div className="flex justify-between items-center mb-4">
                              ...
                              <button onClick={() => { const newBiro = [...biroList]; newBiro[idx].ahli.push("Ahli Baru"); setBiroList(newBiro); }} className="text-xs bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 px-3 py-1.5 border border-slate-200 dark:border-slate-600 rounded-lg flex items-center gap-1 font-bold shadow-sm hover:scale-105 active:scale-95 transition-all"><Plus size={14}/> Tambah</button>
                            </div>
                            <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                              {biro.ahli.map((ahli, aIdx) => (
                                <div key={aIdx} className="flex gap-2">
                                  <input value={ahli} onChange={e => { const newBiro = [...biroList]; newBiro[idx].ahli[aIdx] = e.target.value; setBiroList(newBiro); }} className="flex-1 px-3 py-2 border-2 border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium bg-white dark:bg-slate-800 text-slate-800 dark:text-white outline-none focus:border-blue-500" />
                                  <button onClick={() => { const newBiro = [...biroList]; newBiro[idx].ahli = newBiro[idx].ahli.filter((_, i) => i !== aIdx); setBiroList(newBiro); }} className="text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 px-3 rounded-xl transition-colors"><Trash2 size={18}/></button>
                                </div>
                              ))}
                            </div>
                          </div>
                  ...
                  <div className="flex justify-between items-end mb-6 px-2 shrink-0">
                    ...
                    {isAdmin && (
                       <button onClick={() => {
                         ...
                         setJadualData([...jadualData, { id: newId, tarikh: today, slots: [] }]);
                            ...
                            <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-orange-200 dark:border-orange-900/50 mb-10 flex flex-col sm:flex-row gap-5 items-center justify-between sticky top-[150px] z-20 transition-colors">
                              <div className="w-full">
                                ...
                                <input 
                                  type="date"
                                  value={hari.tarikh} 
                                  onChange={e => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, tarikh: e.target.value } : d))}
                              ...
                              <div className="flex gap-3 w-full sm:w-auto mt-2 sm:mt-0 shrink-0">
                                <button onClick={() => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, slots: [...d.slots, { id: 's' + Date.now(), startTime: "08:00", endTime: "09:00", aktiviti: "Aktiviti Baru" }] } : d))} className="flex-1 sm:flex-none bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-400 px-5 py-3 rounded-xl text-sm font-bold active:scale-95 transition-transform shadow-sm">
                                  + Slot Masa
                                </button>
                                <button onClick={() => {
                                  const newData = jadualData.filter(h => h.id !== hari.id);
                                  setJadualData(newData);
                                  if (newData.length > 0) setActiveJadualTab(newData[0].id);
                                ...
                                <div key={slot.id} className="relative pl-8 md:pl-12 group">
                                  ...
                                  {isAdmin ? (
                                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 relative group-hover:border-orange-400 dark:group-hover:border-orange-600 transition-colors">
                                      <button onClick={() => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, slots: d.slots.filter(s => s.id !== slot.id) } : d))} className="absolute top-5 right-5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 p-2 rounded-xl transition-colors"><Trash2 size={20}/></button>
                                      ...
                                      <div className="mb-4 pr-12 grid grid-cols-2 gap-4">
                                        <div>
                                          ...
                                          <input type="time" value={slot.startTime} onChange={e => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, slots: d.slots.map(s => s.id === slot.id ? { ...s, startTime: e.target.value } : s) } : d))} className="w-full px-4 py-2.5 border-2 border-slate-100 dark:border-slate-700 rounded-xl text-sm font-bold text-orange-600 dark:text-orange-400 bg-slate-50 dark:bg-slate-900 focus:border-orange-500 outline-none" />
                                        </div>
                                        <div>
                                          <label className="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Masa Tamat</label>
                                          <input type="time" value={slot.endTime} onChange={e => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, slots: d.slots.map(s => s.id === slot.id ? { ...s, endTime: e.target.value } : s) } : d))} className="w-full px-4 py-2.5 border-2 border-slate-100 dark:border-slate-700 rounded-xl text-sm font-bold text-orange-600 dark:text-orange-400 bg-slate-50 dark:bg-slate-900 focus:border-orange-500 outline-none" />
                                        </div>
                                      </div>
                                      <textarea value={slot.aktiviti} onChange={e => setJadualData(prev => prev.map(d => d.id === hari.id ? { ...d, slots: d.slots.map(s => s.id === slot.id ? { ...s, aktiviti: e.target.value } : s) } : d))} className="w-full px-5 py-4 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-sm md:text-base font-semibold focus:border-orange-500 outline-none text-slate-800 dark:text-white bg-slate-50 dark:bg-slate-900 custom-scrollbar" rows={3} placeholder="Keterangan Aktiviti..." />
                                    </div>
                                  ) : (
              ...
              {currentView === 'penutup' && (
                <div className="p-4 max-w-4xl mx-auto pb-32 flex flex-col min-h-screen md:min-h-[calc(100vh-100px)]">
                  <div className="flex justify-between items-end mb-6 px-2 shrink-0">
                    ...
                    {isAdmin && (
                       <button onClick={() => setPenutupData([...penutupData, { id: 'p' + Date.now(), time: "08:00", aktiviti: "Aktiviti Baru" }])} className="text-sm bg-rose-500 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 font-bold hover:bg-rose-600 shadow-md transition-all active:scale-95 shrink-0">
                            ...
                            <div key={slot.id} className="relative pl-8 md:pl-12 group">
                              ...
                              {isAdmin ? (
                                <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 relative group-hover:border-rose-400 dark:group-hover:border-rose-600 transition-colors">
                                  <button onClick={() => setPenutupData(penutupData.filter(s => s.id !== slot.id))} className="absolute top-5 right-5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 p-2 rounded-xl transition-colors"><Trash2 size={20}/></button>
                                  
                                  <div className="mb-4 pr-12">
                                    <label className="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Masa</label>
                                    <input type="time" value={slot.time} onChange={e => setPenutupData(penutupData.map(s => s.id === slot.id ? { ...s, time: e.target.value } : s))} className="w-full sm:w-1/2 px-4 py-2.5 border-2 border-slate-100 dark:border-slate-700 rounded-xl text-sm font-bold text-rose-600 dark:text-rose-400 bg-slate-50 dark:bg-slate-900 focus:border-rose-500 outline-none" />
                                  </div>
                                  <textarea value={slot.aktiviti} onChange={e => setPenutupData(penutupData.map(s => s.id === slot.id ? { ...s, aktiviti: e.target.value } : s))} className="w-full px-5 py-4 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-sm md:text-base font-semibold focus:border-rose-500 outline-none text-slate-800 dark:text-white bg-slate-50 dark:bg-slate-900 custom-scrollbar" rows={2} placeholder="Keterangan Aktiviti..." />
                                </div>
                              ) : (
        ...
        <nav className="md:hidden fixed bottom-0 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 flex justify-around items-center pt-2 pb-safe-bottom pb-4 z-40 transition-colors">
          {[
            ...
            <button 
              ...
              className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-all ${currentView === item.id ? 'text-blue-600 dark:text-blue-400 transform -translate-y-1' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
           ...
           {showFabMenu && (
             <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-lg border border-slate-200 dark:border-slate-700 p-2 rounded-3xl shadow-2xl flex flex-col gap-2 animate-in slide-in-from-bottom-5 zoom-in-90 duration-200 origin-bottom-right mb-2">
                ...
                <a href="https://www.facebook.com/ilpsdk" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 rounded-2xl transition-colors text-sm font-bold text-slate-700 dark:text-slate-200">
                  <div className="bg-blue-600 text-white p-1.5 rounded-lg"><Facebook size={16}/></div> Facebook
                </a>
